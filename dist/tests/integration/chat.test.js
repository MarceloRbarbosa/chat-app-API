...
async function signup(username) {
    return (0, supertest_1.default)(app_1.default).post("/auth/signup").send({
        username,
        password: "123456",
...
async function signin(username) {
    return (0, supertest_1.default)(app_1.default).post("/auth/signin").send({
        username,
        password: "123456",
    });
}
async function signupAndSignin() {
    ...
    if (s1.status !== 201 && s1.status !== 409) {
        throw new Error(`Signup failed: ${s1.status} - body=${JSON.stringify(s1.body)} text=${s1.text}`);
    }
    ...
    if (login.status === 404) {
        ...
        if (s2.status !== 201 && s2.status !== 409) {
            throw new Error(`Signup retry failed: ${s2.status} - body=${JSON.stringify(s2.body)} text=${s2.text}`);
        }
        login = await signin(username);
    }
    if (login.status !== 200) {
        throw new Error(`Signin failed: ${login.status} - username=${username} body=${JSON.stringify(login.body)} text=${login.text}`);
    }
    const token = (_a = login.body) === null || _a === void 0 ? void 0 : _a.token;
    if (!token) {
        throw new Error(`Token missing: username=${username} body=${JSON.stringify(login.body)} text=${login.text}`);
    }
    return { token, username };
}
describe("Chat - integration", () => {
    it("POST /chat without token -> 401", async () => {
        const res = await (0, supertest_1.default)(app_1.default).post("/chat").send({
            to: "Todos",
            text: "Oi",
            type: "message",
        });
